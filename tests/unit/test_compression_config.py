...
def test_config_from_env_defaults(monkeypatch):
    ...
    assert cfg.compression_gpu_id == 0
...
def test_config_validate_rejects_negative_gpu_id(tmp_path):
    cfg = CompressionConfig(
        data_dir=tmp_path,
        train_image_root=tmp_path / "train",
        train_metadata_path=tmp_path / "train.csv",
    ...
    for path in (
        ...
        path.mkdir(parents=True, exist_ok=True)
    cfg.train_metadata_path.write_text("image_name;species_id\n")
    cfg.test_csv_path.write_text("plot_id\n")
